...
def anonymous_letter_files(L_file, M_file):
    """Takes 2 filenames as input, which contain text. Returns True if the text in L can be made from words in M, False if it cannot. Case insensitive and punctuation insensitive."""
    L_words = find_word_frequencies(make_all_lowercase(strip_punctuation(open(L_file).read())))
    M_words = find_word_frequencies(make_all_lowercase(strip_punctuation(open(M_file).read())))
